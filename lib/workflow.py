...
BACKUP_TARGET = Path(PureWindowsPath(r'\\10.10.11.31\FI-V3-BIM26 (backup)'))
...
class Workflow:
    ...
    def run(self):
        ...
        self.login_pwd()
        try:
            data_list = self._manager_api.get_resources_by_criterion(self._auth_context, {})
            for item_dict in data_list:
                if item_dict["type"] == "project":
                    self.backup_model(item_dict)
    ...
    def backup_model(self, project_dict):
        # Extract the project name from the project dictionary
        file_name = project_dict["name"]
        ...
        local_path = PureWindowsPath(project_dict["$pathOnServer"])

        # Convert the last modification timestamp from milliseconds to a datetime object
        last_edit_datetime = datetime.fromtimestamp(project_dict["$modifiedDate"] / 1000).replace(tzinfo=None)

        # Construct the path to the backup folder for this project
        model_path = Path(local_path) / BACKUP_FOLDER

        # Construct the target path where the backup will be stored
        target_path = BACKUP_TARGET / bimcloud_path

        # Define the target file path
        target_file = target_path / f"{file_name}{PROJECT_FILE_EXTENSION}"
    ...
    def _perform_backup(self, model_path, target_path, target_file):
        """
        Performs the backup by copying the latest backup file to the target location.

        Args:
            file_name (str): The name of the project file.
        ...
        if not model_path.exists():
            logging.error(f"Source directory does not exist: {model_path}")
            return
        ...
        backup_files = list(model_path.glob(f'*{BACKUP_FILE_EXTENSION}'))
        if not backup_files:
            logging.info(f"No backup files found in {model_path}")
            return

        # Define the latest backup file
        latest_backup_file = max(backup_files, key=lambda f: f.stat().st_mtime)

        # Copy the latest backup file to the target location
        try:
            shutil.copy2(latest_backup_file, target_file)
            logging.info(f"Copied {latest_backup_file} to {target_file}")
        except Exception as e:
            logging.error(f"Error copying {latest_backup_file} to {target_file}: {e}")

    def _verify_backup(self, target_file):
        if target_file.exists():
            modification_time = datetime.fromtimestamp(target_file.stat().st_mtime, timezone.utc)
            if (datetime.now().astimezone(timezone.utc) - modification_time).total_seconds() < 28800:  # 8 hours
                logging.info(f"Backup successfully updated: {target_file}")
            else:
                logging.error(f"Backup file not updated within the last 8 hours: {target_file}")
        else:
            logging.error(f"Backup file does not exist: {target_file}")

    def login_pwd(self):
        logging.info(f"Login as {self.username} ...")
    ...
    def upload_file(self, path, name, alias=None):
        ...
        def do_upload(blob_server_session_id: str, blob_server_api: BlobServerApi):
            ...
            upload = blob_server_api.begin_upload(blob_server_session_id, blob_server_file_path,
                                                  batch['namespace-name'])
    ...
    def convert_timestamp(timestamp):
        return datetime.fromtimestamp(timestamp).strftime("%B %d, %Y %I:%M:%S")
